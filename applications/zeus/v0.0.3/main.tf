...
# Stub source archive uploaded to the shared pegasus functionpackages bucket.
# The lifecycle ignore_changes in the cloudfunction module ensures Terraform
# never overwrites what CI/CD deploys after the first apply.
...
resource "google_storage_bucket_object" "processor_stub" {
  name   = "zeus/processor_stub.zip"
  bucket = var.functionpackages_bucket_name
...
module "processor" {
  ...
  source_archive_bucket = var.functionpackages_bucket_name
